...
if($produto == "HEA" or $produto == "SRAC" or $produto == "MWO"){
	if($produto == "SRAC"){
		...
		$totalAtual = $reparoObj->readTotalbyEstadoNP(1);
		...
		$totalanterior = $reparoObj->readTotalbyEstadoNP(1);
		...
		$reparoObj->nomeproduto = $produto;
		$vendasObj->nomeproduto = $produto;
		$totalAtual = $reparoObj->readTotalbyEstadoNP(2);
		$totalvendaatual = $vendasObj->readbyNP();
		...
		$totalanterior = $reparoObj->readTotalbyEstadoNP(2);
		$totalvendaanterior = $vendasObj->readbyNP();
	}
	else{
	...
	$totalAtual = $reparoObj->readTotalbyEstadoLP();
	...
	$vendasObj->datavenda1 = $data1;
	$vendasObj->datavenda2 = $data2;
	$totalanterior = $reparoObj->readTotalbyEstadoCH();
	$totalvendaanterior = $vendasObj->readbyNP();
	
}elseif($produto == "23L" or $produto == "30L" ){
	$reparoObj->litro = $produto;
	$vendasObj->nomeproduto = "MWO";
	$totalAtual = $reparoObj->readTotalbyEstadoLI();
	$totalvendaatual = $vendasObj->readbyNP();
	$ano = $ano2;
	$ano2 = $ano2 - 1;
	$data1 = $ano2."-".$mes2."-1";
	...
	$vendasObj->datavenda1 = $data1;
	$vendasObj->datavenda2 = $data2;
	$totalanterior = $reparoObj->readTotalbyEstadoLI();
	$totalvendaanterior = $vendasObj->readbyNP();
}	
